...
const RATING_LABEL: Record<PurpleAnnotation['rating'], string> = {
  effective: 'Ефективно',
  neutral: 'Нейтрально',
  ineffective: 'Неефективно',
};
...
const PurpleTeamPanel = () => {
  ...
  const redScore = purpleTimeline.filter(t => t.team === 'red').reduce((s, t) => s + t.pointsGained, 0);
  const blueScore = purpleTimeline.filter(t => t.team === 'blue').reduce((s, t) => s + t.pointsGained, 0);
    ...
    <div className="h-full flex">
      ...
      <div className="w-64 border-r border-border flex flex-col shrink-0">
        <div className="p-3 border-b border-border flex items-center gap-2">
          ...
          <p className="font-display text-xs font-bold text-cyber-purple tracking-wider">ХРОНОЛОГІЯ</p>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1 mb-0.5">
                    ...
                    <span className="text-muted-foreground ml-auto">Хід {entry.turn}</span>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-3">
          ...
          <>
            <div className="flex items-center justify-between">
              <span className={`font-display text-sm font-bold ${step.team === 'red' ? 'text-cyber-red' : 'text-cyber-blue'}`}>
                {step.team === 'red' ? 'RED TEAM' : 'BLUE TEAM'} — Хід {step.turn}
              </span>
            ...
            <div className="p-3 rounded border border-border bg-muted/20 space-y-1">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Дія</p>
              <p className="text-sm font-mono text-foreground">{step.action}</p>
            </div>

            <div className="p-3 rounded border border-border bg-muted/20 space-y-1">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Результат</p>
              <p className="text-sm font-mono text-foreground">{step.result}</p>
              {step.pointsGained > 0 && <p className="text-xs text-cyber-yellow">+{step.pointsGained} очків</p>}
            </div>

            <div className="p-3 rounded border border-cyber-purple/30 bg-cyber-purple/5 space-y-1">
              <p className="text-[10px] text-cyber-purple font-bold uppercase tracking-wider">Авто-аналіз</p>
              <p className="text-xs font-mono text-muted-foreground leading-relaxed">{step.autoInsight}</p>
            </div>

            {/* Annotation form */}
            <div className="p-3 rounded border border-border bg-card space-y-3">
              <p className="font-display text-xs font-bold text-foreground tracking-wider">ТВІЙ ОГЛЯД</p>
              ...
              <Textarea
                ...
                placeholder="Введи рекомендацію або коментар для цього кроку..."
              ...
              <Button onClick={handleSave} size="sm" className="w-full bg-cyber-purple text-white hover:bg-cyber-purple/80">
                {existing ? 'Оновити огляд' : 'Зберегти та перейти далі'}
              </Button>
            </div>

            {existing && (
              <div className="p-3 rounded border border-cyber-green/30 bg-cyber-green/5 space-y-1">
                <p className="text-[10px] text-cyber-green font-bold uppercase tracking-wider">Збережено</p>
                <p className={`text-xs font-bold ${RATING_COLOR[existing.rating].split(' ')[0]}`}>{RATING_LABEL[existing.rating]}</p>
        ...
        {allAnnotated && (
          <div className="p-4 rounded border border-cyber-purple bg-cyber-purple/10 text-center space-y-1">
            ...
            <p className="font-display text-sm font-bold text-cyber-purple">ОГЛЯД ЗАВЕРШЕНО</p>
            <p className="text-xs text-muted-foreground">Всі {totalSteps} кроків проаналізовано. Підсумок — у правій колонці.</p>
          </div>
        )}
      </div>
      ...
      <div className="w-56 border-l border-border p-3 space-y-4 shrink-0 overflow-auto">
        <p className="font-display text-xs font-bold text-cyber-purple tracking-wider">ПІДСУМОК</p>

        <div className="space-y-1">
          <p className="text-[10px] text-muted-foreground uppercase">Прогрес</p>
        ...
        <div className="space-y-1">
          <p className="text-[10px] text-muted-foreground uppercase">Очки Red</p>
          <p className="text-sm font-display font-bold text-cyber-red">{redScore}</p>
        </div>
        <div className="space-y-1">
          <p className="text-[10px] text-muted-foreground uppercase">Очки Blue</p>
            ...
            <div className="text-xs font-mono space-y-0.5">
              {(['effective', 'neutral', 'ineffective'] as const).map(r => (
                <div key={r} className="flex justify-between">
                  <span className={RATING_COLOR[r].split(' ')[0]}>{RATING_LABEL[r]}</span>
