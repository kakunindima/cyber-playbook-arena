  ...
  return (
    <Tabs defaultValue="phishing" className="h-full">
      <TabsList className="bg-muted/30 border border-border mb-4">
        <TabsTrigger value="phishing" className="text-xs gap-1 data-[state=active]:text-cyber-red"><Mail className="w-3 h-3" /> Фішинг</TabsTrigger>
        <TabsTrigger value="social" className="text-xs gap-1 data-[state=active]:text-cyber-red"><MessageSquare className="w-3 h-3" /> Соц.інж.</TabsTrigger>
        <TabsTrigger value="technical" className="text-xs gap-1 data-[state=active]:text-cyber-red"><Terminal className="w-3 h-3" /> Тех.атаки</TabsTrigger>
        <TabsTrigger value="access" className="text-xs gap-1 data-[state=active]:text-cyber-red"><FileKey className="w-3 h-3" /> Доступ</TabsTrigger>
      </TabsList>

      {/* Phishing */}
      <TabsContent value="phishing" className="space-y-4">
        <div className="p-4 rounded border border-border bg-card">
          <h3 className="font-display text-sm font-bold text-cyber-red mb-4">КОНСТРУКТОР ФІШИНГ-ЛИСТА</h3>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Жертва:</label>
              <select
                value={phishTarget}
              ...
              >
                ...
                {state.npcs.map(n => (
                  <option key={n.id} value={n.id}>
                    {n.avatar} {n.name} ({n.role}) — довіра: {n.trustLevel}%
                    {n.isCompromised ? ' ✓' : ''}
                  </option>
                ))}
              </select>
            ...
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Тема листа:</label>
              <Input
                value={phishSubject}
                onChange={e => setPhishSubject(e.target.value)}
                placeholder="Терміново: оновіть свій пароль!"
                className="bg-muted text-foreground"
              />
            </div>
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Текст листа:</label>
              <Textarea
                value={phishBody}
                onChange={e => setPhishBody(e.target.value)}
                placeholder="Шановний колего, просимо підтвердити ваші дані..."
            ...
            >
              <Mail className="w-4 h-4 mr-2" /> Надіслати фішинг-лист
            </Button>
          </div>
        </div>
        ...
        {Object.keys(state.discoveredPasswords).length > 0 && (
          <div className="p-4 rounded border border-cyber-red/30 bg-cyber-red/5">
            <h4 className="text-xs font-display font-bold text-cyber-red mb-2">ОТРИМАНІ ПАРОЛІ</h4>
      ...
      <TabsContent value="social" className="space-y-4">
        <div className="p-4 rounded border border-border bg-card">
          <h3 className="font-display text-sm font-bold text-cyber-red mb-4">СОЦІАЛЬНА ІНЖЕНЕРІЯ</h3>
          <div className="space-y-3">
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Ціль:</label>
              <select
                value={socialTarget}
              ...
              >
                <option value="">— Обрати —</option>
                {state.npcs.map(n => (
                  <option key={n.id} value={n.id}>{n.avatar} {n.name} ({n.role})</option>
                ))}
              </select>
            </div>
            <div>
              <label className="text-xs text-muted-foreground block mb-1">Підхід:</label>
              <div className="flex gap-2">
                {[
                  { val: 'friendly', label: '😊 Дружній' },
                  { val: 'authority', label: '👔 Авторитет' },
                  { val: 'urgency', label: '⚡ Терміново' },
                ].map(a => (
                  <button
                    key={a.val}
            ...
            <Button
              ...
              className="w-full bg-cyber-red text-cyber-red-foreground hover:bg-cyber-red/80"
            >
              <MessageSquare className="w-4 h-4 mr-2" /> Почати розмову
            </Button>
          </div>
        </div>
      </TabsContent>
      ...
      <TabsContent value="technical" className="space-y-4">
        <div className="p-4 rounded border border-border bg-card space-y-3">
          <h3 className="font-display text-sm font-bold text-cyber-red mb-2">ТЕХНІЧНІ АТАКИ</h3>
          ...
          <Button onClick={scanPorts} variant="outline" className="w-full justify-start gap-2 border-cyber-red/30 hover:bg-cyber-red/10 text-foreground">
            <Scan className="w-4 h-4 text-cyber-red" /> Сканувати порти (nmap)
          </Button>

          <div className="border-t border-border pt-3">
            <label className="text-xs text-muted-foreground block mb-1">Брутфорс пароля:</label>
            <select
              value={bruteTarget}
            ...
            >
              <Terminal className="w-4 h-4 text-cyber-red" /> Запустити брутфорс
            </Button>
          </div>
          ...
          <Button onClick={sqlInjection} variant="outline" className="w-full justify-start gap-2 border-cyber-red/30 hover:bg-cyber-red/10 text-foreground">
            <Database className="w-4 h-4 text-cyber-red" /> SQL-ін'єкція
          </Button>
        </div>

        {/* Port scan results */}
        <div className="p-4 rounded border border-border bg-card">
          <h4 className="text-xs font-display font-bold text-foreground mb-2">ПОРТИ СЕРВЕРА</h4>
      ...
      <TabsContent value="access" className="space-y-4">
        <div className="p-4 rounded border border-border bg-card">
          <h3 className="font-display text-sm font-bold text-cyber-red mb-4">ДОСТУП ДО ФАЙЛІВ</h3>
          <div className="mb-4 p-3 rounded bg-muted/30 border border-border">
            <p className="text-xs text-muted-foreground mb-1">Поточний рівень доступу:</p>
            <p className={`font-display font-bold text-lg ${
              ...
              {state.accessLevel === 'none' ? 'Немає доступу' : state.accessLevel.toUpperCase()}
            </p>
          </div>
          ...
          <Button
            ...
            className="w-full bg-cyber-red text-cyber-red-foreground hover:bg-cyber-red/80"
          >
            <FileKey className="w-4 h-4 mr-2" /> Спробувати отримати секретний файл
          </Button>
          {state.accessLevel !== 'admin' && (
            <p className="text-xs text-cyber-yellow mt-2 text-center">
              ⚠ Потрібен рівень admin для доступу до секретного файлу
            </p>
          )}
        </div>
      </TabsContent>
